...
## [Unreleased]
### Changed
- [#89] Update postgresql to v14.24 — the dogu version v14.24-1 was released while the image still built on PostgreSQL 14.23

### Fixed
- [#89] Add `resources/post-upgrade.sh` and `resources/util.sh` to the shellcheck list in the `Jenkinsfile` — both were never linted
- [#89] Expand `${BASH_SOURCE[0]}` instead of `$BASH_SOURCE` when re-executing `post-upgrade.sh` via `gosu` (SC2128)
